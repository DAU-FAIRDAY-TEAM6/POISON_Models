...
def get_data(path = "dataset/"):
    ...
    if len(tmp_business_id) >= 10:
        ...
        user_ratings_list.append(tmp_ratings)
        user_reviews_list.append(tmp_reviews)
    print(len(user_history_list), len(user_ratings_list), len(user_reviews_list))
    
    # history_list를 기준으로 POI에 방문한 사람들 list 생성
    poi_visited_list = []
    for user,history in enumerate(user_history_list):
        for idx, poi in enumerate(history):
            poi_visited_list.append([int(user), int(poi), float(user_ratings_list[user][idx]), user_reviews_list[user][idx]])
            
            
    poi_visited_list.sort(key = lambda x:x[1])
    ...
    for idx, i in enumerate(poi_visited_list):
        user_id, business_id, rating, review = i[0], i[1], i[2], i[3]
        if business_id == before_poi_id: # 이전 POI Id와 동일하다면
            ...
            if int(business_id) - int(before_poi_id) > 1:
                for _ in range(int(business_id) - int(before_poi_id) - 1):
                    #print(f"{business_id},{before_poi_id}들어갔따~")
    ...
    print(len(item_history_list), len(item_ratings_list), len(item_reviews_list))
    return user_history_list, user_ratings_list, user_reviews_list, item_history_list, item_ratings_list, item_reviews_list
